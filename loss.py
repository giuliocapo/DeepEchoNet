...
def silog_loss(pred, target, eps=1e-8, lam=0.85):
    pred = torch.clamp(pred, min=eps)
    target = torch.clamp(target, min=eps)
    d = torch.log(pred) - torch.log(target)
    mse = torch.mean(d ** 2)
    mean = torch.mean(d)
    return torch.sqrt(torch.clamp(mse - lam * (mean ** 2), min=0.0))
...
class balanced_loss_function(nn.Module):
    ...
    def __init__(self, device):
        ...
        self.lambda_3 = 100 
         ...
         # >>> SI-Log <<<
        self.use_silog = False
        self.lambda_silog = 0.3   
        self.silog_lambda = 0.85  
    ...
    def forward(self, output, depth):
        ...
        if self.use_silog:
            loss_silog = silog_loss(output, depth, lam=self.silog_lambda)
            loss_depth = loss_depth + self.lambda_silog * loss_silog
